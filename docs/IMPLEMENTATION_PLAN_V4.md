...
### Tranche 2 delivered
1. Handshake payload now carries optional gossip membership metadata
   (`status`, `last_seen_at_unix`, `vnode_assignments`, `membership`).
2. Gossip probe runtime now advertises local membership and converges to
   runtime-discovered peers instead of probing only static config peers.
3. Peer listener now ingests gossip payload from accepted handshakes and
   merges remote membership views into coordinator state.
4. Vnode ownership propagation is now handled by gossip merge updates and
   reflected in `TOPOLOGY` runtime peer state.

## Phase 45: Multi-writer ack pipeline (v6)

**Status: ✅ Done.**

### Tranche 1 delivered
1. Coordinator replication factor now defaults to `N = min(3, cluster_size)`.
2. Write-owner admission is relaxed for write commands
   (`INSERT` / `UPDATE` / `DELETE` / `PUT_WITH_VC`): the local node may admit
   the write when it belongs to the ring preference list for the key.
3. Read-owner enforcement remains primary-owner only.

### Tranche 2 delivered
1. Coordinator static validation now accepts bounded write concerns
   `W ∈ [1, N]` (with `R` still pinned to `1`).
2. Added key-level runtime write-concern validation for keyed writes:
   requested `W` must be `<=` currently live replicas in the key preference
   list.
3. Liveness semantics for write-concern validation now use peer status:
   `down` is unavailable; `unknown` and `suspect` are treated as potentially
   available.
4. Handler now applies keyed write-concern runtime validation after auth and
   before command execution.
5. Read/non-write commands carrying `WRITE_CONCERN` now return a clear
   validation error.
...
1. Coordinator write execution now fans out acknowledgement waits to replica
    peers over the existing handshake probe transport.
2. Added write acknowledgement timeout control via `write_ack_timeout_ms`.
3. Partial-ack and timeout outcomes now fail quorum wait explicitly.
4. Handler now blocks write execution success when quorum wait fails.
5. Added coordinator tests covering quorum success and timeout failure paths.
